...
_SKILLS_DIR = REPO_ROOT / "src" / "mujoco_mcp" / "skills"


@mcp.resource("skill://{name}")
def get_skill(name: str) -> str:
    """Get a skill's markdown content by name.

    Skills teach an LLM how to use the MuJoCo server effectively.
    """
...
@mcp.prompt()
def debug_crashed_sim(job_id: str) -> str:
    """Guide to debug a crashed simulation from logs."""
    return f"""You are debugging simulation job {job_id}.

1. Read the runner log: analyze_sim_logs(job_id="{job_id}")
2. Check if the model is valid: list_models()
3. If the root cause is a model issue, try a different model
4. If it's a rendering issue, restart with headless=True"""


@mcp.prompt()
def tune_control_policy(job_id: str) -> str:
    """Guide to tune actuator controls for a running sim."""
    return f"""You are tuning control parameters for simulation job {job_id}.

1. Read the current state: get_state(job_id="{job_id}")
2. Analyze the robot posture: analyze_sim_state(job_id="{job_id}")
3. Suggest control values for each actuator
4. Apply with: apply_control(job_id="{job_id}", ctrl={{...}})
5. Re-read state and iterate"""


@mcp.prompt()
def compare_two_models(model_a: str, model_b: str) -> str:
    """Guide to compare two loaded models in the depot."""
    return f"""Comparing models '{model_a}' and '{model_b}'.

1. Check both models exist: list_models()
2. Start both sims simultaneously
3. Compare joint counts, body counts, actuator counts
4. Analyze differences in complexity and capabilities"""
...
@mcp.tool(app=True)
async def show_sim_status_card() -> dict:
    """Show server health, MuJoCo version, and active jobs as a rich card.

    ## Return Format
    {"success": bool, "mujoco_version": str|None, "models_in_depot": int, "active_jobs": int}
...
@mcp.tool(app=True)
async def show_models_card() -> dict:
    """Show all loaded models as a rich card with metadata.
    ...
    depot = _load_depot()
    with PrefabApp(title="Model Depot") as app:
        Heading(f"{len(depot)} Models Loaded")
        for name, info in list(depot.items())[:20]:
            meta = info.get("metadata", {})
            parts = f"{meta.get('joint_count', '?')}j {meta.get('body_count', '?')}b {meta.get('actuator_count', '?')}a"
            Row(label=name, value=parts)
    return {"content": f"Models: {len(depot)}", "structured_content": app}


@mcp.tool(app=True)
async def show_jobs_card() -> dict:
    """Show active and completed simulation jobs as a rich card.
