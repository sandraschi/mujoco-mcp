...
const BACKEND_NAME: &str = "mujoco-mcp-backend.exe";
const BACKEND_PORT: u16 = 11046;
const BACKEND_TAG: &str = "mujoco-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "MUJOCO_MCP_PORT";
const ENV_HOST: &str = "MUJOCO_MCP_HOST";
const ENV_TAURI: &str = "MUJOCO_MCP_TAURI";
...
fn free_port(port: u16) -> bool {
    ...
    {
        let img_kill = format!(
            "Stop-Process -Name 'mujoco-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'mujoco-mcp-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM mujoco-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM mujoco-mcp-native.exe /T 2>$null"
        );
            ...
            if i == 15 {
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name mujoco-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM mujoco-mcp-backend.exe /T 2>$null; \
                     Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                     ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
                );
