...
# Gate: verify entry point exists before PyInstaller (catches silent 0-byte exe)
$entryFile = "$Root\run_server.py"
if (-not (Test-Path $entryFile)) {
    throw "run_server.py not found at $entryFile - the spec file references this as the entry point. Create run_server.py with dual-transport (MCP_PORT → HTTP, fallback → stdio) before building."
}
...
# Use project venv pyinstaller, not global
...
# Gate: smoke-test the frozen binary (catches import crashes generically)
$frozenExe = "$Root\dist\${RepoName}-backend.exe"
Write-Host "  Smoke-testing frozen binary..." -ForegroundColor Yellow
$testPort = 11999
$oldPort = $env:MUJOCO_MCP_PORT; $oldHost = $env:MUJOCO_MCP_HOST
$env:MUJOCO_MCP_PORT = "$testPort"; $env:MUJOCO_MCP_HOST = "127.0.0.1"
$testProc = Start-Process -FilePath $frozenExe -NoNewWindow -PassThru -RedirectStandardError "$Root\dist\pyi-crash.log"
Start-Sleep -Seconds 5
$env:MUJOCO_MCP_PORT = $oldPort; $env:MUJOCO_MCP_HOST = $oldHost
...
$strayExe = "$PSScriptRoot\target\release\mujoco-mcp-backend.exe"
