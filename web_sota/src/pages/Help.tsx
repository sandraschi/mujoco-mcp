      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {TABS.map((t, i) => (
          <button
            key={t}
            onClick={() => setTab(i)}
            className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${tab === i ? "bg-cyan-700 text-white" : "border border-slate-600 text-slate-400 hover:bg-slate-800"}`}
          >
            {t}
          </button>
        ))}
    ...
    <div className="space-y-4">
      <Card title="What It Is">
        <p className="text-sm text-slate-400 mb-2">
          ...
          communicating over a JSON pipe for crash isolation.
        </p>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 text-left text-slate-500">
              <th className="pb-2 pr-4 font-medium text-slate-400">Port</th>
              <th className="pb-2 font-medium text-slate-400">Service</th>
            </tr>
          </thead>
          <tbody>
            <tr className="border-b border-slate-700">
              <td className="py-2 pr-4 text-xs font-mono text-cyan-400">11046</td>
              <td className="py-2 text-xs text-slate-400">FastAPI backend + MCP HTTP (+ FastMCP stdio)</td>
            </tr>
            <tr>
              <td className="py-2 pr-4 text-xs font-mono text-cyan-400">11047</td>
              <td className="py-2 text-xs text-slate-400">Vite React frontend (dev)</td>
            </tr>
          </tbody>
        </table>
      </Card>
      ...
      <Card title="Core Simulation Tools (9)">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 text-left text-slate-500">
                <th className="pb-2 pr-4 font-medium text-slate-400">Tool</th>
            ...
            <tbody>
              {sim.map((t) => (
                <tr key={t.name} className="border-b border-slate-700">
                  <td className="py-2 pr-4 text-xs font-mono text-cyan-400 whitespace-nowrap">{t.name}</td>
                  <td className="py-2 text-xs text-slate-400">{t.desc}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Card title="Prerequisites">
        <ul className="list-disc list-inside text-sm text-slate-400 space-y-1">
          ...
          <li>
            ...
            <code className="text-xs bg-slate-700 px-1 rounded text-slate-300">pip install mujoco</code> (3.2+)
          </li>
          <li>
          ...
          <li>
            ...
            <code className="text-xs bg-slate-700 px-1 rounded text-slate-300">pip install uv</code>
          </li>
          <li>
...
uv run python -m mujoco_mcp`}
        ...
        <p className="text-xs text-slate-500 mt-2">
          Or use <code className="text-xs bg-slate-700 px-1 rounded text-slate-300">.\start.bat</code> (backend +
          webapp) or <code className="text-xs bg-slate-700 px-1 rounded text-slate-300">.\start.ps1 -Headless</code>{" "}
          (backend only). After starting, open the <strong>Models</strong> page and click the{" "}
          <strong>MuJoCo Menagerie</strong> tab to browse and download 100+ robot models.
        </p>
      </Card>
      ...
      <Card title="Configuration">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              <tr className="border-b border-slate-700">
                <td className="py-2 pr-4 text-xs font-mono text-cyan-400">MUJOCO_MCP_DEPOT</td>
                <td className="py-2 pr-4 text-xs text-slate-500">~/.mujoco-mcp/models/</td>
                <td className="py-2 text-xs text-slate-400">Model depot directory</td>
              </tr>
              <tr className="border-b border-slate-700">
                <td className="py-2 pr-4 text-xs font-mono text-cyan-400">MUJOCO_MCP_PORT</td>
                <td className="py-2 pr-4 text-xs text-slate-500">11046</td>
                <td className="py-2 text-xs text-slate-400">MCP server HTTP port</td>
              </tr>
              <tr>
                <td className="py-2 pr-4 text-xs font-mono text-cyan-400">OLLAMA_URL</td>
                <td className="py-2 pr-4 text-xs text-slate-500">http://localhost:11434</td>
                <td className="py-2 text-xs text-slate-400">Ollama for AI tool fallback</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <Card title="Common Issues">
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 text-left text-slate-500">
              <th className="pb-2 pr-4 font-medium text-slate-400">Symptom</th>
              <th className="pb-2 pr-4 font-medium text-slate-400">Cause</th>
              <th className="pb-2 font-medium text-slate-400">Fix</th>
            </tr>
          </thead>
          <tbody>
            {TROUBLES.map((t, i) => (
              <tr key={i} className="border-b border-slate-100">
                <td className="py-2 pr-4 text-xs text-red-400 font-medium align-top">{t.symptom}</td>
                <td className="py-2 pr-4 text-xs text-slate-400 align-top">{t.cause}</td>
                <td className="py-2 text-xs text-slate-300 font-mono align-top whitespace-pre-wrap">{t.fix}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="mt-4 p-3 bg-slate-700 rounded text-xs text-slate-400">
        <p className="mb-1">
          ...
          <code className="text-xs bg-slate-800 px-1 rounded">jobs/&lt;job_id&gt;/runner.log</code> — per-simulation
          subprocess stderr
        </p>
        <p className="mb-1">
          <strong>Reset:</strong> Delete <code className="text-xs bg-slate-800 px-1 rounded">jobs/</code> and{" "}
          <code className="text-xs bg-slate-800 px-1 rounded">models/.depot/registry.json</code> to clear all state
        </p>
      </div>
    </Card>
  );
