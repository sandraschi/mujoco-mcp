...
export default function Models() {
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      const d = await r.json();
      setMessage(`Seeded ${d.seeded.length}.${d.failed.length ? ` ${d.failed.length} failed: ${d.failed.map((f: any) => f.name).join(", ")}` : ""}`);
      fetchModels();
    } catch (e) { setMessage(String(e)); }
    setSeeding(false);
  };
  ...
  return (
    <div className="max-w-5xl" data-testid="models-page">
      ...
      <div className="flex gap-2 mb-6">
        ...
        <button onClick={() => setTab("menagerie")} className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${tab === "menagerie" ? "bg-cyan-700 text-white" : "border border-slate-600 text-slate-400 hover:bg-slate-800"}`}>MuJoCo Menagerie</button>
      </div>
        ...
        <>
          <div className="bg-slate-800 rounded-xl p-5 border border-slate-700 mb-6" data-testid="load-model-form">
            ...
            <div className="flex flex-col gap-3">
              <input className="bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-cyan-500" placeholder="Model name (e.g. my_robot)" value={name} onChange={(e) => setName(e.target.value)} data-testid="model-name-input" />
              <input className="bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-cyan-500" placeholder="URL or local file path" value={uri} onChange={(e) => setUri(e.target.value)} data-testid="model-uri-input" />
              <div className="flex gap-3">
                <button onClick={handleLoad} disabled={loading || !name || !uri} className="bg-cyan-700 hover:bg-cyan-600 disabled:bg-slate-600 text-white px-4 py-2 rounded-lg text-sm font-medium" data-testid="load-model-btn">{loading ? "Loading..." : "Load Model"}</button>
                <button onClick={handleSeed} disabled={seeding} className="bg-green-700 hover:bg-green-600 disabled:bg-slate-600 text-white px-4 py-2 rounded-lg text-sm font-medium" data-testid="seed-models-btn">{seeding ? "Seeding..." : "Seed Quick Models"}</button>
              </div>
            </div>
          ...
          <div className="bg-slate-800 rounded-xl border border-slate-700" data-testid="model-list">
            <h2 className="text-lg font-semibold p-4 border-b border-slate-700">Loaded Models ({Object.keys(models).length})</h2>
      ...
      {tab === "menagerie" && (
        <div className="bg-slate-800 rounded-xl p-5 border border-slate-700">
          ...
          <p className="text-xs text-slate-400 mb-4">Browse and download models from the <a href="https://github.com/google-deepmind/mujoco_menagerie" target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:underline">MuJoCo Menagerie</a>.</p>
          <input
          ...
          {!menagerieLoading && menagerie.length === 0 && <div className="text-sm text-slate-500">No models found.</div>}
